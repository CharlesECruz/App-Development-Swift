...
class FilterBarCollectionView: UICollectionView {
    ...
    var foodieDelegate: FoodDelegate?
    ...
    @objc private func initUI() {
        categories.append(FoodController.shared.categoriesTimes)
        categories.append(FoodController.shared.foodCategories)
        filters.append(.init(repeating: false, count: categories[FoodController.TIME_INDEX].count))
        filters.append(.init(repeating: false, count: categories[FoodController.FOOD_INDEX].count))
        reloadData()
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        for (type, filter) in filters.enumerated() {
            for (index, filtered) in filter.enumerated() {
                if filtered, let restaurants = FoodController.shared.restaurants(in: categories[type][index], of: type) {
                    filteredRestaurants[type].append(contentsOf:restaurants)
                }
            }
        }
        let timeTypeSet = Set(filteredRestaurants[FoodController.TIME_INDEX].map { $0 })
        let foodTypeSet = Set(filteredRestaurants[FoodController.FOOD_INDEX].map { $0 })
        if timeTypeSet.isEmpty && foodTypeSet.isEmpty {
            foodieDelegate?.filterItem(FoodController.shared.restaurants)
